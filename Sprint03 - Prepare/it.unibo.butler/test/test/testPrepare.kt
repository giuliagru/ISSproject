...
class testPrepare {
	...
	@After
	fun terminate() {
		println("testPrepare terminated ")
	}
	...
	fun testPrep(){
 		runBlocking{
			...
			checkResource(clientrobotexec, "startprepare", "robotexecutor")
			checkResource(clientrobotmover, "moving", "robotmover")
			
			delay(1000)
			checkResource(clientrobot, "handlingprepare", "robot")
			checkResource(clientrobotexec, "atpantry", "robotexecutor")
			checkResource(clientrobotmover, "moving", "robotmover")
			
			delay(1000)
			checkResource(clientrobot, "handlingprepare", "robot")
			checkResource(clientrobotexec, "attable_dishes", "robotexecutor")
			checkResource(clientrobotmover, "moving", "robotmover")
